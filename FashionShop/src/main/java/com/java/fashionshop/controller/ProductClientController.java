...
public class ProductClientController {
	...
	public ResponseEntity<?> getTopNewestProductsWithVariants(@RequestParam(defaultValue = "0") int page,
	...
	public ResponseEntity<?> getProductDetail(@PathVariable Integer id) {
		...
		if (entity == null) {
			// Trả về 200 OK, body null
			return ResponseEntity.ok(null);
		}
		return ResponseEntity.ok(productService.convertToDTO(entity));
	}
	...
	public ResponseEntity<List<ProductVariantDTO>> getVariants(@PathVariable Integer id) {
		...
		List<ProductVariantDTO> result = variants.stream().map(this::convertToVariantDTO).toList();
		return ResponseEntity.ok(result);
	}
	...
	public ResponseEntity<?> getVariantByColorAndSize(@PathVariable Integer productId, @RequestParam Integer colorId,
	...
	public ResponseEntity<?> getProductOptions(@PathVariable Integer id) {
		...
		List<ColorsDTO> colors = variants.stream().filter(v -> v.getColor() != null).map(v -> new ColorsDTO())
				.distinct().toList();

		// Lấy các size không trùng
		List<SizesDTO> sizes = variants.stream().filter(v -> v.getSize() != null).map(v -> new SizesDTO()).distinct()
				.toList();
	...
	public ResponseEntity<Page<ProductDTO>> getRelatedProducts(@RequestParam Integer categoryId,
			@RequestParam(required = false) Integer excludeProductId, @RequestParam(defaultValue = "0") int page,
